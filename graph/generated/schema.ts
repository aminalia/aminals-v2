...
export class PendingBirth extends Entity {
  constructor(id: Bytes) {
    super();
    this.set("id", Value.fromBytes(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save PendingBirth entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type PendingBirth must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`,
      );
      store.set("PendingBirth", id.toBytes().toHexString(), this);
    }
  }

  static loadInBlock(id: Bytes): PendingBirth | null {
    return changetype<PendingBirth | null>(
      store.get_in_block("PendingBirth", id.toHexString()),
    );
  }

  static load(id: Bytes): PendingBirth | null {
    return changetype<PendingBirth | null>(
      store.get("PendingBirth", id.toHexString()),
    );
  }
  ...
  set parentOne(value: Bytes) {
    this.set("parentOne", Value.fromBytes(value));
  }
  ...
  set parentTwo(value: Bytes) {
    this.set("parentTwo", Value.fromBytes(value));
  }

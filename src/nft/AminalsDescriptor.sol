...
abstract contract AminalsDescriptor is NFTDescriptor {
    ...
    function dataURI(uint256 tokenId) public view returns (string memory) {
        ...
        string memory image = string(abi.encodePacked('data:image/svg+xml;base64,', Base64.encode(bytes(_aminalImage(tokenId, 0, 0, 0, 0, 0, 0, 0)))));
        if ( tokenId == 1 ) {
            image = string(abi.encodePacked('data:image/svg+xml;base64,', Base64.encode(bytes(_aminalImage(tokenId, 1, 1, 1, 1, 1, 1, 1)))));
        }
        if ( tokenId == 2 ) {
            image = string(abi.encodePacked('data:image/svg+xml;base64,', Base64.encode(bytes(_aminalImage(tokenId, 1, 0, 1, 0, 1, 0, 1)))));
        }
        string memory description = string(abi.encodePacked('This NFT represents a digital pet. This NFT cannot be transfered.'));
     ...
     */
    function _aminalImage(uint _tokenId, uint _background, uint _tail, uint _arm, uint _ear, uint _body, uint _face, uint _mouth) internal view returns (string memory output) {
        // Aminal Base - for all Aminals
        output = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 1000 1000"><style>.cls-1 { opacity: .06; } .cls-1, .cls-2 { fill: #fff; } .cls-3 { fill: #';
        output = string(abi.encodePacked(output, backgrounds[0], '; } .cls-4 { fill: #f57f2f; } .cls-5 { fill: #001a2a; } .cls-6 { fill: #eba220; } .cls-7 { fill: #fffbd0; } .cls-8 { fill: #b8171c; } .cls-9 { fill: #923018; } .cls-10 { fill: #e01f26; } .cls-11 { fill: #b22024; } .cls-12 { fill: #0a3035; } .cls-13 { fill: #1c4349; } .cls-14 { fill: #7c0506; } .cls-15 { fill: #e97935; } .cls-16 { fill: #e01f26; } .cls-17 { fill: #f5d235; } .cls-18 { fill: #f15a29; } .cls-19 { fill: #e01f26; } .cls-20 { fill: #e01f26; } .cls-21 { fill: #3c3d55; opacity: .52; }  </style> <g id="BG"><rect class="cls-3" x="0" y="0" width="1000" height="1000"/> </g><g id="shadow"><ellipse class="cls-21" cx="505" cy="971" rx="163" ry="12"/></g>'));
        
        output = string(abi.encodePacked(output, tails[_tail]));
        output = string(abi.encodePacked(output, arms[_arm]));
        output = string(abi.encodePacked(output, ears[_ear] ));
        output = string(abi.encodePacked(output, bodies[_body] ));
        output = string(abi.encodePacked(output, faces[_face]));
        output = string(abi.encodePacked(output, mouths[_mouth]));
    ...
    // eg. 00a79d
    function addBackground(string memory background) public returns (bool success) {
    ...
    function addArm(string memory arm) public returns (bool success) {
        arms.push(arm);
        return true;
    }    
    ...
    function addTail(string memory tail) public returns (bool success) {
        tails.push(tail);
        return true;
    }    
    ...
    function addEar(string memory ear) public returns (bool success) {
        ears.push(ear);
        return true;
    }
    ...
    function addBody(string memory body) public returns (bool success) {
        bodies.push(body);
        return true;
    }
    // Aminal #0 - Face
    ...
    function addFace(string memory face) public returns (bool success) {
        faces.push(face);
        return true;
    }
    ...
    function addMouth(string memory mouth) public returns (bool success) {
        mouths.push(mouth);
        return true;
    }
